/*---------------------------------------------------------------------------------------
  Скрипт для обновления базы до версии 0.9.4

  Что нового:
	+ Дата последней активности клиента LastActivityDate
...
SET @PreviousVersion = '0.9.3' 			-- номер ПРЕДЫДУЩЕЙ версии
SET @NewVersion     = '0.9.4'			-- номер новой версии
...
alter table Client add LastActivityDate datetime null
...
update Client
set LastActivityDate = CreationDate
GO
IF @@ERROR <> 0 AND @@TRANCOUNT > 0 BEGIN PRINT 'Шаг окончен неуспешно.' ROLLBACK TRAN END
GO
IF @@TRANCOUNT = 0 BEGIN PRINT 'Дальше выполнять ничего не будем' SET NOEXEC ON END
...
alter table Client alter column LastActivityDate datetime null
-- ПОСЛЕ КАЖДОЙ ОПЕРАЦИИ
GO
IF @@ERROR <> 0 AND @@TRANCOUNT > 0 BEGIN PRINT 'Шаг окончен неуспешно.' ROLLBACK TRAN END
GO
