/*---------------------------------------------------------------------------------------
  Скрипт для обновления базы до версии 0.9.2

  Что нового:	
	* Столбцы, относящиеся к почтовому адресу клиента, сделаны nullable
...
SET @PreviousVersion = '0.9.1' 			-- номер ПРЕДЫДУЩЕЙ версии
SET @NewVersion     = '0.9.2'			-- номер новой версии
...
alter table [Client] alter column PostalPostalIndex VARCHAR(6) null
GO
...
alter table [Client] alter column PostalLocalAddress VARCHAR(100) null
GO
IF @@ERROR <> 0 AND @@TRANCOUNT > 0 BEGIN PRINT 'Шаг окончен неуспешно.' ROLLBACK TRAN END
GO
IF @@TRANCOUNT = 0 BEGIN PRINT 'Дальше выполнять ничего не будем' SET NOEXEC ON END
...
alter table [Client] alter column PostalCityId SMALLINT null
GO
IF @@ERROR <> 0 AND @@TRANCOUNT > 0 BEGIN PRINT 'Шаг окончен неуспешно.' ROLLBACK TRAN END
GO
IF @@TRANCOUNT = 0 BEGIN PRINT 'Дальше выполнять ничего не будем' SET NOEXEC ON END
...
drop table ClientAccountConfiguration
GO
IF @@ERROR <> 0 AND @@TRANCOUNT > 0 BEGIN PRINT 'Шаг окончен неуспешно.' ROLLBACK TRAN END
GO
IF @@TRANCOUNT = 0 BEGIN PRINT 'Дальше выполнять ничего не будем' SET NOEXEC ON END
