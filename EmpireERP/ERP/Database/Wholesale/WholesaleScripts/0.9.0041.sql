/*---------------------------------------------------------------------------------------
  Скрипт для обновления базы до версии 0.9.41

  Что нового:
	+ Поле Name для ProductionOrderBatch. Name = "Партия от " + CreationDate
	+ Добавление прав на создание и удаления накладной
	+ Разделеным партиям устанавливаем дату создания = дате создания родителя
	* Переименование этапа "Создания заказа" в "Создание"
...
SET @PreviousVersion = '0.9.40' 			-- номер ПРЕДЫДУЩЕЙ версии
SET @NewVersion     = '0.9.41'			-- номер новой версии
...
alter table dbo.[ProductionOrderBatch] ADD  Name VARCHAR(200) null
...
update dbo.[ProductionOrderBatch] 
set Name = 'Партия от ' + convert(varchar(100), CreationDate, 104) + ' ' + convert(varchar(100), CreationDate, 108)
...
alter table dbo.[ProductionOrderBatch] alter column Name VARCHAR(200) not null
...
---Всем разделенным партиям выставляем дату создания родителя 
update P
set CreationDate = T.MinCreationDate, Date = T.MinCreationDate
from ProductionOrderBatch P
join 
(
	select ProductionOrderId, MinCreationDate = MIN(CreationDate)
	from ProductionOrderBatch 
	group by ProductionOrderId 
	HAVING count(ProductionOrderId) > 1
) T on T.ProductionOrderId = P.ProductionOrderId
...
UPDATE DefaultProductionOrderBatchStage SET Name = 'Создание' WHERE Name='Создание заказа'
...
UPDATE ProductionOrderBatchStage SET Name = 'Создание' WHERE Name='Создание заказа'
