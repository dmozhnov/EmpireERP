/*---------------------------------------------------------------------------------------
  Скрипт для обновления базы до версии 0.9.67

  Что нового:
	+ добавлена хранимая процедура GetAvailableArticles 
	* изменена хранимая процедура Report0002 : теперь данные фильтруются не по
	группам товаров, а по товарам 
...
SET @PreviousVersion = '0.9.66' 			-- номер ПРЕДЫДУЩЕЙ версии
SET @NewVersion     = '0.9.67'			-- номер новой версии
...
/*********************************************************************************************
Процедура: 	Report0002

Описание:	Получение данных для отчета Report0002 в виде плоской таблицы

Параметры:
	@StartDate Дата начала диапазона
	@EndDate Дата завершения диапазона
	@IsShippedExpenditureWaybills Признак выборки накладных в финальном статусе(иначе проведенные)
	@DevideByBatch Признак разделения партий
	@GetArticleAvailability Признак необходимости подсчитать наличие на МХ
	@InAccountingPrice Признак вывода УЦ
	@ConsiderReturnFromClient Признак, нужно ли учитывать возвраты
	@ConsiderReturnFromClientByDate Признак того, что нужно учитывать возвраты из указанного интервала дат. Иначе из указанных возвратов.
	@StorageIdList Список кодов МХ
	@AllStorages Признак выбора всех МХ
	@TakeArticlesFromArticleGroup Признак того, откуда брать коды товары. Если 1 , то из параметров @ArticleGroupIdList или @AllArticleGroups.Иначе из @ArticleIdList.
	@ArticleGroupIdList Список кодов групп товаров
	@AllArticleGroups Признак выбора всех групп товаров
	@ArticleIdList --- Список кодов товаров
	@ClientIdList Список кодов клиентов
	@AllClients Признак выбора всех клиентов
	@UserIdList Список кодов групп пользователей
	@AllUsers Признак выбора всех пользователей
	@AccountOrganizationIdList Список кодов организаций аккаунта
	@AllAccountOrganizations Признак выбора всех организаций аккаунта
	@UserId Код пользователя, запросившего отчет
