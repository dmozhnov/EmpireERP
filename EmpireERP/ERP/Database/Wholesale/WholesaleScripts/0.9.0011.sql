/*---------------------------------------------------------------------------------------
  Скрипт для обновления базы до версии 0.9.11

  Что нового:
	
	+ к накладной продажи (SaleWaybill) добавлено поле "форма оплаты" (PaymentFormId)
	* выставляет форму оплаты ранее созданных продаж по следующему алгоритму:
		1. По продаже нет оплат - форма оплаты = 2 (безналичный расчет)
		2. По продаже есть оплаты - форма оплаты равна форме оплаты первой (по дате) оплате по этой продаже
...
SET @PreviousVersion = '0.9.10' 			-- номер ПРЕДЫДУЩЕЙ версии
SET @NewVersion     = '0.9.11'			-- номер новой версии
...
GO
	...
	add PaymentFormId tinyint NOT NULL
	constraint DF_PaymentFormId default (2) 
		
	alter table SaleWaybill
	drop constraint DF_PaymentFormId
...
GO
	...
	set PaymentFormId = 
	(
		select top 1 PaymentFormId 
		from Payment p 
		join PaymentDistribution pd on pd.PaymentId = p.Id 
		join PaymentDistributionToSale pdts on pdts.Id = pd.Id 
		and pdts.SaleWaybillId = SaleWaybill.Id
		order by [Date]
	)
	where exists (
				select Id from PaymentDistributionToSale pdts where pdts.SaleWaybillId = SaleWaybill.Id
				)
