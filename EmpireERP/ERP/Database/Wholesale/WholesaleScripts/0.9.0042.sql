/*---------------------------------------------------------------------------------------
  Скрипт для обновления базы до версии 0.9.42

  Что нового:
	+ Права на группу отчетов 0010 и операцию «Смена пользователя в оплатах»
	+ Поле UserId в DealPaymentDocument
...
SET @PreviousVersion = '0.9.41' 			-- номер ПРЕДЫДУЩЕЙ версии
SET @NewVersion     = '0.9.42'			-- номер новой версии
...
INSERT INTO PermissionDistribution (PermissionDistributionTypeId, PermissionId, RoleId) VALUES (3, 24901, 1);
GO
...
alter table dbo.[DealPaymentDocument] add UserId INT null
GO
IF @@ERROR <> 0 AND @@TRANCOUNT > 0 BEGIN PRINT 'Шаг окончен неуспешно.' ROLLBACK TRAN END
GO
IF @@TRANCOUNT = 0 BEGIN PRINT 'Дальше выполнять ничего не будем' SET NOEXEC ON END
...
update [DealPaymentDocument] set UserId = 1
GO
IF @@ERROR <> 0 AND @@TRANCOUNT > 0 BEGIN PRINT 'Шаг окончен неуспешно.' ROLLBACK TRAN END
GO
IF @@TRANCOUNT = 0 BEGIN PRINT 'Дальше выполнять ничего не будем' SET NOEXEC ON END
...
alter table dbo.[DealPaymentDocument] alter column UserId INT not null
GO
IF @@ERROR <> 0 AND @@TRANCOUNT > 0 BEGIN PRINT 'Шаг окончен неуспешно.' ROLLBACK TRAN END
GO
IF @@TRANCOUNT = 0 BEGIN PRINT 'Дальше выполнять ничего не будем' SET NOEXEC ON END
...
alter table dbo.[DealPaymentDocument] add constraint FK_DealPaymentDocument_User foreign key (UserId) references dbo.[User]
GO
IF @@ERROR <> 0 AND @@TRANCOUNT > 0 BEGIN PRINT 'Шаг окончен неуспешно.' ROLLBACK TRAN END
GO
IF @@TRANCOUNT = 0 BEGIN PRINT 'Дальше выполнять ничего не будем' SET NOEXEC ON END
...
INSERT INTO PermissionDistribution (PermissionDistributionTypeId, PermissionId, RoleId) VALUES (3, 3706, 1)
GO
IF @@ERROR <> 0 AND @@TRANCOUNT > 0 BEGIN PRINT 'Шаг окончен неуспешно.' ROLLBACK TRAN END
GO
IF @@TRANCOUNT = 0 BEGIN PRINT 'Дальше выполнять ничего не будем' SET NOEXEC ON END
