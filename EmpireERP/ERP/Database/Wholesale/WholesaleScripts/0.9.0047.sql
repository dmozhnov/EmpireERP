/*---------------------------------------------------------------------------------------
  Скрипт для обновления базы до версии 0.9.47

  Что нового:
	+ права для накладной реализации «Изменение даты накладной», «Проводка задним числом», «Отгрузка задним числом»
	+ поле DistributionDate в DealPaymentDocumentDistribution
...
SET @PreviousVersion = '0.9.46' 			-- номер ПРЕДЫДУЩЕЙ версии
SET @NewVersion     = '0.9.47'			-- номер новой версии
...
alter table Deal add StartDate datetime null
GO
IF @@ERROR <> 0 AND @@TRANCOUNT > 0 BEGIN PRINT 'Шаг окончен неуспешно.' ROLLBACK TRAN END
GO
IF @@TRANCOUNT = 0 BEGIN PRINT 'Дальше выполнять ничего не будем' SET NOEXEC ON END
...
alter table Deal alter column StartDate datetime not null
GO
IF @@ERROR <> 0 AND @@TRANCOUNT > 0 BEGIN PRINT 'Шаг окончен неуспешно.' ROLLBACK TRAN END
GO
IF @@TRANCOUNT = 0 BEGIN PRINT 'Дальше выполнять ничего не будем' SET NOEXEC ON END
...
alter table DealPaymentDocumentDistribution add DistributionDate datetime null
GO
IF @@ERROR <> 0 AND @@TRANCOUNT > 0 BEGIN PRINT 'Шаг окончен неуспешно.' ROLLBACK TRAN END
GO
IF @@TRANCOUNT = 0 BEGIN PRINT 'Дальше выполнять ничего не будем' SET NOEXEC ON END
...
set DistributionDate = CreationDate
GO
IF @@ERROR <> 0 AND @@TRANCOUNT > 0 BEGIN PRINT 'Шаг окончен неуспешно.' ROLLBACK TRAN END
GO
IF @@TRANCOUNT = 0 BEGIN PRINT 'Дальше выполнять ничего не будем' SET NOEXEC ON END
...
alter table DealPaymentDocumentDistribution alter column DistributionDate datetime not null
GO
IF @@ERROR <> 0 AND @@TRANCOUNT > 0 BEGIN PRINT 'Шаг окончен неуспешно.' ROLLBACK TRAN END
GO
IF @@TRANCOUNT = 0 BEGIN PRINT 'Дальше выполнять ничего не будем' SET NOEXEC ON END
