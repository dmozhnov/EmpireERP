/*---------------------------------------------------------------------------------------
  Скрипт для обновления базы до версии 0.9.45

  Что нового:
	* Размеры упаковки товара больше не могут быть null. Null заменен на ноль.
	* Рассчитан объем для товаров, по которым известны все три линейных размера.
...
SET @PreviousVersion = '0.9.44' 			-- номер ПРЕДЫДУЩЕЙ версии
SET @NewVersion     = '0.9.45'			-- номер новой версии
...
alter table Article alter column PackHeight decimal(6, 0) NOT NULL
...
alter table Article alter column PackLength decimal(6, 0) NOT NULL
...
alter table Article alter column PackWidth decimal(6, 0) NOT NULL
...
UPDATE Article
SET
	PackVolume = (PackHeight * PackLength * PackWidth) / 1000000000
WHERE
	PackVolume = 0
	AND PackHeight <> 0
	AND PackLength <> 0
