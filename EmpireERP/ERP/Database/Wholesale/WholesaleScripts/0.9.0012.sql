/*---------------------------------------------------------------------------------------
  Скрипт для обновления базы до версии 0.9.12

  Что нового:
	+ в ProductionOrderBatchRow добавлено поле ArticleMeasureUnitScale - 
		количество знаков после запятой для единицы измерения товара
	+ в ProductionOrderBatchRow добавлено поле PackSize - 
		количество товара в упаковке на момент создания позиции. 
		Для заполнения старых позиций значения берутся из справочника товаров.
	* изменен тип поля PackSize в таблице Article - было smallint, стало decimal(12,6)
...
SET @PreviousVersion = '0.9.11' 			-- номер ПРЕДЫДУЩЕЙ версии
SET @NewVersion     = '0.9.12'			-- номер новой версии
...
GO
	...
	ADD ArticleMeasureUnitScale TINYINT not null
	CONSTRAINT DF_ProductionOrderBatchRow_ArticleMeasureUnitScale DEFAULT 0
	ALTER TABLE dbo.ProductionOrderBatchRow DROP CONSTRAINT DF_ProductionOrderBatchRow_ArticleMeasureUnitScale
...
GO
	...
	FROM [dbo].[ProductionOrderBatchRow] R
...
GO
	...
	ALTER TABLE Article
	ALTER COLUMN PackSize decimal(12,6) NOT NULL
...
update ProductionOrderBatchRow
set PackSize = (
	select top 1 PackSize 
	from Article
	where Article.Id = ProductionOrderBatchRow.ArticleId)
