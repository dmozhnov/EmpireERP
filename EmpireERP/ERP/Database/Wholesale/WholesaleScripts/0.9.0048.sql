/*---------------------------------------------------------------------------------------
  Скрипт для обновления базы до версии 0.9.48

  Что нового:
	+ поле GigabyteCountLimit в Setting
...
SET @PreviousVersion = '0.9.47' 			-- номер ПРЕДЫДУЩЕЙ версии
SET @NewVersion     = '0.9.48'			-- номер новой версии
...
alter table Setting add GigabyteCountLimit smallint null
GO
...
alter table Setting alter column GigabyteCountLimit smallint not null
GO
IF @@ERROR <> 0 AND @@TRANCOUNT > 0 BEGIN PRINT 'Шаг окончен неуспешно.' ROLLBACK TRAN END
GO
IF @@TRANCOUNT = 0 BEGIN PRINT 'Дальше выполнять ничего не будем' SET NOEXEC ON END
