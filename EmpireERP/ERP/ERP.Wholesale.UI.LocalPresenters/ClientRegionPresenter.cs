...
{
    public class ClientRegionPresenter : BaseDictionaryPresenter<ClientRegion>, IClientRegionPresenter
    {
        #region Поля
        ...
        public ClientRegionPresenter(IUnitOfWorkFactory unitOfWorkFactory, IClientRegionService clientRegionService, IUserService userService)
            : base(clientRegionService, userService)
        {
        ...
        public GridData GetClientRegionGrid(GridState state, UserInfo currentUser)
        {
            using (IUnitOfWork uow = unitOfWorkFactory.Create(IsolationLevel.ReadCommitted))
            {
        ...
        public object GetClientRegions()
        {
            using (IUnitOfWork uow = unitOfWorkFactory.Create(IsolationLevel.ReadCommitted))
            {
        ...
        public BaseDictionaryEditViewModel Create(UserInfo currentUser)
        {
            using (IUnitOfWork uow = unitOfWorkFactory.Create(IsolationLevel.ReadCommitted))
            {
                ...
                model.Title = "Добавление региона клиента";
        ...
        public BaseDictionaryEditViewModel Edit(short id, UserInfo currentUser)
        {
            using (IUnitOfWork uow = unitOfWorkFactory.Create(IsolationLevel.ReadCommitted))
            {
                ...
                model.Title = model.AllowToEdit ? "Редактирование региона клиента" : "Детали региона клиента";
        ...
        public object Save(BaseDictionaryEditViewModel model, UserInfo currentUser)
        {
            ...
            {
                var clientRegion = base.SaveBaseDictionary(model, currentUser);
